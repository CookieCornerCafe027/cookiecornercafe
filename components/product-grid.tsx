...
export function ProductGrid({ products }: ProductGridProps) {
  const handleAddToCart = (productId: string) => {
    ...
    if (product) {
      const size =
        product.price_small ? "small" : product.price_medium ? "medium" : "large"
  ...
  return (
    <div className="space-y-12">
      {crepeCakes.length > 0 && (
        <div>
          <h3 className="text-2xl font-display font-semibold mb-6 text-primary">Crepe Cakes</h3>
      ...
      {cookies.length > 0 && (
        <div>
          <h3 className="text-2xl font-display font-semibold mb-6 text-primary">Cookies & Treats</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cookies.map((product) => (
              <ProductCard key={product.id} product={product} onAddToCart={handleAddToCart} />
            ))}
